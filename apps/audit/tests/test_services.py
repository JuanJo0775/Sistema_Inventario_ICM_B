...
def test_login_success_logged(almacenista_user):
    ...
    assert AuditLog.objects.filter(event_type=AuditEventType.LOGIN_SUCCESS).count() >= before + 1
...
def test_movement_creation_logged(almacenista_user, sample_product, sample_locations):
    ...
    assert AuditLog.objects.filter(event_type=AuditEventType.MOVEMENT_CREATED, movement=m).exists()
...
def test_audit_log_metadata_mutable_in_memory(almacenista_user):
    ...
    log = log_event(AuditEventType.LOGOUT, description="Cierre", user=almacenista_user, detail={})
    log.metadata = {"x": 1}
